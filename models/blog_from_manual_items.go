...
var (
	...
	blogFromManualItemGeneratedColumns      = []string{}
)
...
// CountG returns the count of all BlogFromManualItem records in the query using the global executor
...
func BlogFromManualItems(mods ...qm.QueryMod) blogFromManualItemQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`blog_from_manual_items`.*"})
	}

	return blogFromManualItemQuery{NewQuery(q)}
}
